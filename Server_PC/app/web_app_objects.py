...
#This version creates objects for each camera and starts a loop for each one.
#When used from gunicorn, it creates as many workers as cameras, so each camera is processed by a different worker.
#The problem is that each worker has its own set of cameras and streams, so MUCH SLOWER and CPU CONSUMING.
